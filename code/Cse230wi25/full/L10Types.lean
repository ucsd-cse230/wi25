...
theorem add_step : ∀ {Γ a1 a2 s v1 v2 τ},
  (Γ ⊧ s) -> (Γ ⊢ a1 : τ) -> (Γ ⊢ a2 : τ) ->
  ...
  cases τ
  . case TInt =>
    cases h1'; cases h2'; simp_all; constructor; apply T_AddI; repeat assumption
  . case TFloat =>
    cases h1'; cases h2'; simp_all; constructor; apply T_AddF; repeat assumption
...
theorem add_progress: ∀ {Γ a1 a2 τ s},
  ...
  ∃ v, Taval (a1 + a2) s v
  := by
  intros Γ a1 a2 τ s gs g1 g2 av1 av2
  cases av1; cases av2; apply add_step; repeat assumption
...
-- almost same as `add_step` ...

theorem less_step : ∀ {Γ a1 a2 s v1 v2 τ},
  (Γ ⊧ s) -> (Γ ⊢ a1 : τ) -> (Γ ⊢ a2 : τ) ->
  (Taval a1 s v1) -> (Taval a2 s v2) ->
  ∃ v, Tbval (a1 << a2) s v
  := by
  ...
  cases τ
  . case TInt =>
    cases h1'; cases h2'; simp_all; constructor; apply T_LessI; repeat assumption
  . case TFloat =>
    cases h1'; cases h2'; simp_all; constructor; apply T_LessF; repeat assumption
...
theorem bool_progress: ∀ { Γ s } { b : Bexp },
  ...
  . case Ty_Less a1 a2 τ Ga1 Ga2 =>
    ...
    cases h1; cases h2; apply less_step; repeat assumption
...
theorem state_preservation : ∀ { Γ } { c c' : Com } { s s' },
  (Γ ⊢ c) -> (Γ ⊧ s) -> Step (c, s) (c', s') -> (Γ ⊧ s')
  := by
...
theorem preservation : ∀ { Γ } { c c' : Com } { s s' : State },
  (Γ ⊧ s) -> (Γ ⊢ c) -> Step (c, s) (c', s') -> (Γ ⊢ c')
  := by
...
theorem progress1 : ∀ { Γ s } { c : Com},
  (Γ ⊧ s) -> (Γ ⊢ c) ->  ¬ (c = Skip) -> ∃ c' s', Step (c, s) (c', s')
  := by
...
theorem progress : ∀ { Γ } { c c' : Com } { s s' : State},
  ...
  . case step =>
    ...
    cases h
    cases hh
    have ty_c2 : HasTyC Γ c2 := by apply (preservation wf_s ty_c step1)
